...
exports.default = KeySpotLanding;
...
function KeySpotLanding() {
    const skillUrl = "https://raw.githubusercontent.com/roadsidedev/keyspot-sdk/main/SKILL.md";
    const [copiedNpm, setCopiedNpm] = (0, react_1.useState)(false);
    const [copiedSkill, setCopiedSkill] = (0, react_1.useState)(false);
    const handleCopyNpm = () => {
    ...
    return (<div className="min-h-screen bg-white dark:bg-zinc-950 text-zinc-950 dark:text-white">
      {/* Nav */}
      <nav className="border-b border-zinc-200 dark:border-zinc-800">
        <div className="max-w-5xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-6 text-sm">
            <link_1.default href="/doc" className="hover:text-zinc-600 dark:hover:text-zinc-400 transition-colors">Docs</link_1.default>
            <link_1.default href="/pricing" className="hover:text-zinc-600 dark:hover:text-zinc-400 transition-colors">Pricing</link_1.default>
            <a href="https://github.com/roadsidedev/keyspot-sdk" target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 hover:text-zinc-600 dark:hover:text-zinc-400 transition-colors">
              GitHub <lucide_react_1.ExternalLink className="h-3.5 w-3.5"/>
            </a>
          </div>
        </div>
      </nav>
      ...
      <div className="max-w-5xl mx-auto px-6 pt-20 pb-16 text-center">
        ...
        <div className="mt-10 max-w-lg mx-auto space-y-3">
          <div className="group relative flex items-center gap-3 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900 px-5 py-4 font-mono text-sm">
            ...
            <button onClick={handleCopyNpm} className="opacity-60 group-hover:opacity-100 transition p-2 hover:bg-zinc-200 dark:hover:bg-zinc-800 rounded relative" title="Copy to clipboard">
              {copiedNpm ? (<span className="text-xs font-medium text-green-600 dark:text-green-400 absolute -top-8 right-0 whitespace-nowrap">Copied!</span>) : null}
          ...
          <div className="group relative flex items-center gap-3 rounded-xl border border-dashed border-zinc-300 dark:border-zinc-700 bg-zinc-50/50 dark:bg-zinc-900/50 px-5 py-4 font-mono text-sm overflow-x-auto">
            ...
            <button onClick={handleCopySkill} className="opacity-60 group-hover:opacity-100 transition p-2 hover:bg-zinc-200 dark:hover:bg-zinc-800 rounded flex-shrink-0 relative" title="Copy to clipboard">
              {copiedSkill ? (<span className="text-xs font-medium text-green-600 dark:text-green-400 absolute -top-8 right-0 whitespace-nowrap">Copied!</span>) : null}
              <lucide_react_1.Copy className="h-4 w-4"/>
            </button>
          </div>
        </div>
        ...
        <div className="mt-8 flex flex-wrap justify-center gap-4">
          <link_1.default href="/doc" className="inline-flex h-11 items-center justify-center rounded-full bg-zinc-950 dark:bg-white px-6 text-sm font-medium text-white dark:text-zinc-950 transition hover:bg-zinc-800 dark:hover:bg-zinc-200">
            <lucide_react_1.BookOpen className="mr-2 h-4 w-4"/> Read the docs
          </link_1.default>
          <link_1.default href="/pricing" className="inline-flex h-11 items-center justify-center rounded-full border border-zinc-200 dark:border-zinc-800 px-6 text-sm font-medium transition hover:bg-zinc-50 dark:hover:bg-zinc-900">
            <lucide_react_1.CreditCard className="mr-2 h-4 w-4"/> View pricing
          </link_1.default>
        </div>
      </div>
      ...
      <div className="border-t border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/50">
        ...
        ].map((item) => (<div key={item} className="flex items-center gap-3 text-zinc-600 dark:text-zinc-400">
      ...
      <div className="border-t border-zinc-200 dark:border-zinc-800">
        <div className="max-w-5xl mx-auto px-6 py-16">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {features.map((f) => {
            const Icon = f.icon;
            return (<div key={f.title} className="space-y-3">
                  <div className="flex items-center gap-3">
      ...
      <div className="border-t border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/50">
        ...
        ].map((item) => (<div key={item} className="px-4 py-3 rounded-lg border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 text-zinc-700 dark:text-zinc-300">
                {item}
              </div>))}
          </div>
        </div>
      ...
      <footer className="border-t border-zinc-200 dark:border-zinc-800 mt-auto">
        <div className="max-w-5xl mx-auto px-6 h-16 flex items-center justify-between text-xs text-zinc-500 dark:text-zinc-400">
          ...
          <a href="https://github.com/roadsidedev/keyspot-sdk" className="flex items-center gap-1.5 hover:text-zinc-700 dark:hover:text-zinc-300">
            <lucide_react_1.GitBranch className="h-3.5 w-3.5"/> roadsidedev/keyspot-sdk
          </a>
        </div>
      </footer>
    </div>);
