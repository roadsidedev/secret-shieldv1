...
async function main() {
    ...
    await benchmark('checkpoint (small object, 1 secret)', iterations, () => guard.checkpoint(smallObj));
    // 2. Clean object (no secrets)
    const cleanObj = { user: 'alice', message: 'hello world', count: 42 };
    await benchmark('checkpoint (clean object)', iterations, () => guard.checkpoint(cleanObj));
    // 3. Large deep object (100+ nodes)
    const largeObj = generateLargeObject(4, 5);
    await benchmark('checkpoint (large object ~500 nodes)', iterations, () => guard.checkpoint(largeObj));
    // 4. String scan only
    await benchmark('scan (string with secret)', iterations, () => guard.scan('sk-123456789012345678901234567890123456789012345678'));
    // 5. Vault operations
