...
import { KeySpotError, VaultError, WorkerError, AuthError, PaymentRequiredError, ConfigurationError, ValidationError, ScanError, isKeySpotError, toStatusCode, } from '@roadsidelab/keyspot-core/errors';
...
describe('KeySpot Fail-Closed', () => {
    ...
    it('vault write failure during checkpoint blocks state', async () => {
        ...
        const throwingVault = {
            ...
            list: async () => [],
            delete: async () => false,
            generateRef: (id) => `vault:v1:${id}:abc:0`,
            verifyRef: () => false,
        };
...
describe('KeySpot.createSecure', () => {
    ...
    it('creates a KeySpot instance with production defaults', () => {
        const guard = KeySpot.createSecure({ vault: persistentVault() });
    ...
    it('rejects missing vault with ConfigurationError', () => {
        expect(() => KeySpot.createSecure({})).toThrow();
    });
